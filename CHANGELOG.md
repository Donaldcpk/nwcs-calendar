...
### Added
- 日子類型 **SH**（School Holiday，學校假期），與 PH 一併計入 90 天配額；舊 `Holiday` 自動轉 SH。
- SDEC 2026-27 預設假期與活動；日曆活動 **×** 刪除。
- 新增活動僅高亮顏色，不再自動改為 `Event` 類型。
- 校長檢視：空白 SH 不顯示；換月前每月類型日數統計閘門。
- **下載 Excel 存檔**（校曆明細、每月統計、學年摘要三工作表）。
