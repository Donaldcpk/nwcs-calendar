  ...
  return (
    <section className="h-screen overflow-auto bg-slate-50 p-4" onMouseUp={onMouseUp}>
      ...
      <p className="mb-4 text-[11px] text-slate-500">
        多選：拖曳範圍；Shift+點擊區間；Ctrl / ⌘ +點擊切換單日。每月 1 日有橫幅分隔，跨月同一列左側有紫色邊線。
      </p>
      ...
      <div className="grid grid-cols-7 gap-2">
        ...
        {entries.map((day, index) => {
          ...
          const prevMonth = index > 0 ? format(parseISO(entries[index - 1].date), "yyyy-MM") : null;
          const thisMonth = format(dateObj, "yyyy-MM");
          const crossesMonthInRow = isMonthStart && prevMonth !== null && prevMonth !== thisMonth;

          return (
            <Fragment key={day.date}>
              {isMonthStart ? (
                <div
                  className="col-span-7 flex items-center gap-2 rounded-md border border-violet-200 bg-violet-50 px-3 py-1.5"
                  aria-label={`進入${format(dateObj, "yyyy年M月")}`}
                >
                  <span className="text-xs font-bold tracking-wide text-violet-800">{format(dateObj, "yyyy年M月")}</span>
                  <span className="h-px flex-1 bg-violet-200" />
                </div>
              ) : null}
              <button
                type="button"
                className={`relative min-h-24 rounded-lg border p-2 pb-6 text-left text-xs shadow-sm transition ${badgeByType[day.type]} ${selected ? "ring-2 ring-blue-500 ring-offset-1" : "hover:border-blue-400"} ${crossesMonthInRow ? "border-l-4 border-l-violet-500" : ""}`}
